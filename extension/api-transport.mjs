...
async function probeOutputMode(service,protocol,signal){
  ...
  if(protocol==='chat'){
    ...
    if(choice?.finish_reason!=='stop'||choice.message?.refusal)return 'json_object';
    text=choice.message?.content;
  }else{
    if(value.status!=='completed')return 'json_object';
  ...
  let result;try{result=parseObject(text);}catch{return 'json_object';}
...
// CommandCode accepts reasoning.effort on its Responses route but rejects "none" on its
// chat route, so thinking is switched off here only.
function responsesThinking(service){const model=service.model||'';if(service.providerId==='open-responses'||service.providerId==='commandcode')return {reasoning:{effort:'none'}};if(service.providerId==='xai'&&/^grok-4\.3(?:$|-)/i.test(model))return {reasoning:{effort:'none'}};return (service.providerId==='openai'||service.providerId==='azure')&&/^gpt-(?:[5-9]|[1-9]\d)/i.test(model)?{reasoning:{effort:'none'}}:{};}
...
function normalizedModels(items,idOf,nameOf=idOf){const seen=new Set(),models=[];for(const item of items||[]){const id=idOf(item);if(typeof id!=='string'||!id.trim()||seen.has(id))continue;seen.add(id);const name=nameOf(item);const endpoints=item?.supported_endpoints?.filter(e=>['/chat/completions','/responses','/messages'].includes(e))||[];models.push({id,name:typeof name==='string'&&name.trim()?name:id,endpoints});}return models.sort((a,b)=>a.name.localeCompare(b.name));}
// `options.protocol` exists only for providers that declare the field: normalizeApiService
// rejects unknown options, so a stored protocol cannot retarget a provider that lacks it.
